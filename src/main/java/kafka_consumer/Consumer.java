...
public class Consumer 
{
	...
	static void runMainLoop(String[] args, Properties properties) throws InterruptedException, UnsupportedEncodingException {
		...
		// Create Kafka comsumer
		KafkaConsumer<String, String> consumer = new KafkaConsumer<String, String>(properties);
		
		try {
			//Exemplo lendo 2 bombas
			consumer.subscribe(Arrays.asList("Bomba1","Bomba2"));
  	      	
			System.out.println("Subscribed to topic Bomba1 and Bomba2");	
			...
			while (true) 
			{
  	      		...
  	      		for (ConsumerRecord<String, String> record : records){
  	            	System.out.printf("partition = %s, offset = %d, key = %s, value = %s\n", record.partition(), record.offset(), record.key() ,decodeMsg(record.value()).getId());
  	      		}
			}
		}
		finally {
